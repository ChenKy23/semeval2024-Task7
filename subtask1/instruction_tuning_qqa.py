...
def predict_and_save_res(args, tokenizer=None, tokenized_dataset=None, dataset_test=None):
    ...
    decoded_preds, decoded_preds_out = get_predict(model=model, tokenized_dataset = tokenized_dataset, batch_size=25, max_new_tokens=25, sample_set='test', device = 'cuda')

    labels = [0 if sample["answer"].startswith("Option 1") else 1 for sample in dataset_test]
    ...
    save_res = [{"question": sample["question"], "option1": sample["Option1"], "option2": sample['Option2'], "answer": sample['answer']} for sample in dataset_test]
...
def run(args):
    def preprocess_function(sample):
        if args.is_digit_base:
            inputs = [input_template.format(question=question, option1=option1, option2=option2) for question, option1, option2 in zip(sample['question_char'], sample["Option1"], sample["Option2"])]
        else:
            inputs = [input_template.format(question=question, option1=option1, option2=option2) for question, option1, option2 in zip(sample['question'], sample["Option1"], sample["Option2"])]
        
        model_inputs = tokenizer(inputs, truncation=False)

        labels = []
        for answer, option1, option2 in zip(sample["answer"], sample['Option1'], sample['Option2']):
            if '1' in answer:
    ...
    if args.task == "train":
        ...
        tokenized_dataset = datasets.map(preprocess_function, batched=True, remove_columns=["question", "question_char", "Option1", "Option2", "answer"])
