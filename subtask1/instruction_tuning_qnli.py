...
def preprocess_function(sample):
    ...
    # labels = [dict_num_to_str[magnitude] for magnitude in sample["magnitude"]]
...
# 添加需要的参数
parser.add_argument("--data_train_pth", default='/root/semeval2024-A40/Quantitative-101/QNLI/RedditNLI.json', help="dataset's path")
# parser.add_argument("--data_test_pth", default='/home/chenkaiyuan/semeval2024/Quantitative-101/QP/Numeracy600K_comment_dev.json', help="dataset's path")
parser.add_argument("--model_name", default='google/flan-t5-base', help="model name")
parser.add_argument("--seed", default=3407, help="set seed")
# parser.add_argument("--model_checkpoint", default='./model_finetuning_reasoning_ck', help="model checkpoint's path")
parser.add_argument("--model_checkpoint", default='/root/autodl-tmp/QNLI-flan-ft-base-AWPNLI/checkpoint-300', help="model checkpoint's path")
parser.add_argument("--output_file_name", default="save_qnli_res_AWPNLI.json", help="output file's name")
...
tokenized_dataset = dataset.map(preprocess_function, batched=True, remove_columns=["statement1_char", "statement2_char", "options", "answer"])
...
training_args = Seq2SeqTrainingArguments(
    output_dir='/root/autodl-tmp/QNLI-flan-ft-base-AWPNLI',
...
#         output_ids = model.generate(input_ids=inputs, attention_mask=attention_mask, max_new_tokens=max_new_tokens, top_p = 0.9, temperature=0.7)
...
# # f1_metric = evaluate.load("./semeval2024-task7/f1.py")
# f1_metric = evaluate.load("/root/semeval2024-A40/f1.py")
...
# labels = [covert_dict.get(sample["answer"].strip().lower()) for sample in dataset["test"]]
...
def predict_and_save_res(args, tokenizer=None, tokenized_dataset=None, dataset_test=None):
    ...
    f1_metric = evaluate.load("/root/semeval2024-A40/semeval2024-Task7/subtask1/f1.py")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="training code")
    parser.add_argument("--data_train_pth", default='/root/semeval2024-A40/Quantitative-101/QNLI/RedditNLI.json', help="dataset_train's path")
    parser.add_argument("--data_dev_pth", default='/root/semeval2024-A40/Quantitative-101/QP/Numeracy600K_comment_dev.json', help="dataset_dev's path")
    parser.add_argument("--data_test_pth", default='/root/semeval2024-A40/Quantitative-101/QP/Numeracy600K_comment_test.json', help="dataset_test's path")
    parser.add_argument("--num_splits", default=10, help="num of splits")
    parser.add_argument("--is_digit", default=True, help="whether to use digit")
    ...
    parser.add_argument("--model_checkpoint", default='/root/autodl-tmp/qp_flan_t5/checkpoint-306', help="model checkpoint's path")
    ...
    parser.add_argument('--output_model_path', type=str, default='/root/autodl-tmp/qp_flan_t5')
